...
Concurrently, the bonds between the deoxyribose/phosphate groups are also broken as $\UvrD$ progresses. We write these reactions as two separate concerted actions transitions for clarity, with the first transition in full,  and note that this is the first time we have used the shortcut  \rulename{concert3} SOS rule.
\begin{flalign*}
...
\end{flalign*}
Notice that promotion has moved a bond from $s_{11}$ to $p3_{11}$ and from $s$ to $v$. Instead of using \rulename{concert3}, we could have used this sequence of transitions from the source to the target process:
