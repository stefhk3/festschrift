...
We also need additional components, namely the methyl group Me, and the MutS, MutL, MutH, and UvrD proteins. They are defined in CCB as follows:
$$\begin{array}{lll}
\Me & \bydef & (m).(n).\Me' \\
...
The mismatch here is on an A and G pair.
...
A mismatch in the DNA section in Figure~\ref{fig:state1} is between $A_2$ and $G_3$: they do not form an
A-T or G-C pair as required. 
...
Now it is possible for $\MutL$ to bond to $\MutS$ on $l$ with key 30 (see Figure~\ref{fig:state15}):
%
\begin{flalign*}
...
Next, $\MutH$ is ready to cleave the DNA at the right location. This is done by bonding to $\DP_9$ with the key 33 and, at the same time, breaking the bond between $\DP_9$ and $\DP_8$ (with the key 13). These two simultaneous reactions are represented by the following concerted actions transition.
%
\begin{figure}[h!]
\psfrag{dp1}{$DP_1$}
\psfrag{dp2}{$DP_2$}
...
\end{flalign*}
Next, {\em promotion} happens, which moves the bond with the key 33 from a weak action $s_9$ to a strong action $p3_9$ in one of the sites of $\DP_9$:
\begin{flalign*}
...
&((p3_{11}[15],p5_{11}[16];s_{11}) \paral (b_{11}[21],d_{11})).DP_{11}' \paral ((p3_{12}[16],p5_{12};s_{12}) \paral (b_{12}[22],d_{12})).DP_{12}' \paral  &&\\
&(m[40]).(n[31]).Me'\paral (k[28],k[29]).(l[30]).MutS' \paral (l[30]).(n[31]).(o[32]).MutL' \paral &&\\
&(o[32]).({w[33]}).MutH' \paral ((u;r) \paral (v;s)).UvrD'&&
\end{flalign*}
Note that execution of $s_9$ on $\DP_9$ leads to breaking a bond between two deoxyribose/phosphate groups. Specifically, bond 33 from $\MutH$ to $DP_9$ via actions $w$ and $s$ is formed and bond 13 between $p5$ in $\DP_8$ and $p3$ in  $\DP_9$ is broken. %We use concerted actions and rewriting for this. 

The situation after this step is shown in Figure~\ref{fig:state2}. This has now the right DNA strand cleaved.
...
\psfrag{UD}{${\mathit{UvrD}}$}
  ...
  The mismatch has been detected by $\MutS$, and $\MutH$ and $\MutL$ have been recruited. $\MutL$ has detected the methylation, and $\MutH$ has cleaved the strand containing the wrong base.}
  \label{fig:state2}
\end{figure}
...
$\DP_9$ (key 14). Secondly, $\UvrD$ combines with $T_2$ (key 35) thus breaking the bond between $T_2$ and its paired base $A_1$ (key 24). Each of these concerted actions transitions is followed immediately by an application of \rulename{prom}, which moves bonds from weak actions $s_{10}$ and $r_5$ to strong actions $p3_{10}$ and $t_2$ respectively.
...
&(b_8[21];i_8).(g_3[29];r_8).G_3' \paral ((p3_8[12],p5_8;s_8) \paral (b_8[18],d_8)).\DP_8' \paral &&\\
&((p3_9[33],\bm{p5_9};s_9) \paral (b_9[19],d_9)).\DP_9' \paral ((\bm{p3_{10}[34]},p5_{10}[15];\bm{s_{10}}) \paral (b_{10}[20],d_{10})).\DP_{10}' \paral  &&\\
&((p3_{11}[15],p5_{11}[16];s_{11}) \paral (b_{11}[21],d_{11})).\DP_{11}' \paral ((p3_{12}[16],p5_{12};s_{12}) \paral (b_{12}[22],d_{12})).\DP_{12}' \paral  &&\\
&(m[40]).(n[31]).\Me'\paral (k[28],k[29]).(l[30]).\MutS' \paral (l[30]).(n[31]).(o[32]).\MutL' \paral &&\\
...
bold lines show the bonds being created and the dashed lines indicate the bonds being broken.
...
\psfrag{UD}{${\mathit{UvrD}}$}
  ...
  \caption[A six base pair DNA fragment.]{%A six base pair DNA fragment, with a DNA mismatch and methylation of one strand. 
  $\UvrD$ has now bonded to $T_2$ and $\DP_{10}$ (bold lines), breaking the bonds of those molecules to the rest of the DNA (dotted lines). %In a next step, $\UvrD$ can now ``walk'' along the DNA strand, and remove the next pair of $\DP$ and base molecules.
}
  \label{fig:state3}
\end{figure}
...
&(m[40]).(n[31]).\Me'\paral (k[28],k[29]).(l[30]).\MutS' \paral (l[30]).(n[31]).(o[32]).\MutL' \paral &&\\
&(o[32]).(w[33]).\MutH' \paral ((u[35];r) \paral (\bm{v[36]};\bm{s})).\UvrD'&&
\end{flalign*}
...
$\DP_9$ have been removed (shown in grey) and $\UvrD$ is moving to the next  base $C_3$
...
\psfrag{UD}{${\mathit{UvrD}}$}
  ...
  \caption[A six base pair DNA fragment.]{%A six base pair DNA fragment, with a DNA mismatch and methylation of one strand. 
  $\UvrD$ has now bonded to $C_3$ and $\DP_{11}$ (bonds shows as bold lines), breaking the bonds of those molecules to the rest of the DNA via concerted actions (shown as dotted lines). The group of $T_2$ and $\DP_{9}$ is now no longer attached to the DNA and is therefore shown in grey. % This process can continue for more pairs.
}
  \label{fig:state4}
\end{figure}

This process continues until the offending base $G_3$ has been removed. We do not show concerted actions transitions that model that since they correspond very closely to the last two transitions. Figure~\ref{fig:state6} shows the result of these two transitions which remove of the offending base.
...
\psfrag{UD}{${\mathit{UvrD}}$}
  ...
  \caption[A six base pair DNA fragment.]{%A six base pair DNA fragment, with a DNA mismatch and methylation of one strand. 
$\UvrD$ has now bonded to $G_3$ (the offending base) and $\DP_{12}$, breaking the bonds of those molecules to the rest of the DNA via concerted actions (shown as dotted lines). The offending base is removed after this step. As before, the $C_3$-$\DP_{10}$ group is shown in grey, since it is no longer attached to the DNA.}
  \label{fig:state6}
\end{figure}
