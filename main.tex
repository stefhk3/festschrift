...
%
% Not much is written about $Me$ and how it binds with $A$s. Are any other compounds that can bind similarly with $T, G$ and $C$?
% I am not sure using parallel with $A$ is appropriate. Also, from page 24 onwards the actions of copies of $A$ are wrong: weak 
% actions r are with m and not with a as in definitions on page 20. Couldn't Me bond with A_1 on r and thus break bond with T_2 (by concerted
% action)?
%
...
% Actions of As are different here than as defined on page 20.
%
\paral &&\\
...
%
%
%Can you please write the two pair of transitions separately? It will be easier to check and explain to the reader
%

\begin{flalign*}
&\xrightarrow{\{ss[36],\underline{vp3}[34],\underline{p}[15]\}} \overset{ \rulename{prom}}\Rightarrow \xrightarrow{\{rr[37],\underline{tu}[35],\underline{cg}[25]\}} \overset{ \rulename{prom}}\Rightarrow &&\\
...
&(p3_7,p5_7[12];s_7).DP_7' \paral (b_7[17],d_7).DP_7'' \paral (p3_8[12],p5_8;s_8).DP_8' \paral (b_8[18],d_8).DP_8'' \paral &&\\
&(p3_9[33],p5_9;s_9).DP_9' \paral (b_9[19],d_9).DP_9'' \paral (p3_{10},\mathbf{p5_{10}};s_{10}).DP_{10}' \paral (b_{10}[20],d_{10})).DP_{10}'' \paral &&\\
...
&(m[40]).(n[31]).Me'\paral (m2[28],m2[29]).(l[30]).MutS' \paral (l[30]).(n[31]).(o[32]).MutL' \paral &&\\
&(o[32]).(w[33]).MutH' \paral (\mathbf{u[37]};r).UvrD' \paral (\mathbf{v[36]};s).UvrD''&&
\end{flalign*}
...
%
% Remove this comment, or adopt it so it describes a ``new'' problem and not the old one which we may have solved by three action % concerted actions.
%
...
%
% What happens to $Me$ here, as we would like it to un-bond with $A_1$ in Figur 15.
%
...
%
% Also, explain why UD does not continue bonding to DP13 and to G4 and to the other such compounds to the right of the DNA 
% strand.
%
