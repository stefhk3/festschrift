...
%
Calculus of Covalent Bonding, or CCB for short, was introduced in~\cite{KU16} and fully defined in~\cite{KU2017}. We recall some of the main definitions in order to make the paper more self-contained. We note, however, that some SOS rules 
...
Molecules may have several bonding \emph{sites} and several bonds can be created or dissolved at each bonding site. A site and its potential bonds are modelled via $(s;b)$, where $s$ is a sequence of actions and $b$ is a weak action. The action $b$ can be omitted, in which case the construct is written as $(s)$. When a molecule has several sites 
