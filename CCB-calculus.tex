...
Molecules may have several bonding \emph{sites} and several bonds can be created or dissolved at each bonding site. A site and its potential bonds is modelled via a construct $(s;b)$, where $s$ is a sequence of actions and $b$ is a weak action. The action $b$ can be omitted, in which case the construct is written as $(s)$. When a molecule has several sites $(s;b), \ldots,(s';b')$, called a \emph{collection},
which can form bonds with sites of other molecules, we shall write them as $(s;b) \Mid  \ldots \Mid (s';b')$ using the symbol $\paral$ to indicate that sites bond independently of each other. Such expressions will be used to define the prefix operator. More formally, a site $\sigma$ is either $(s;b)$ or $(s)$. A collection $\mu$ is either $\sigma$ or $\sigma\Mid\sigma$, and 
...
extends the prefixing operators in \cite{DK2007}, \cite{Irek2012}, and {\cite{KU16}. 
If each site of $\mu$ has only one action, then it is the multi-action prefix from \cite{DK2007}.
If the collection $\mu$ has only one site, then it is the general prefixing operator from \cite{KU16}. 
If it and the site has no weak action, then the prefixing is written as $(s).P$ and is called the
\emph{simple prefix}. 
...
undoing one of the past actions in $s$ (by the \rulename{concert} rules in Figures~\ref{fig:c1sos}-\ref{fig:c2sos}).
...
respectively in Figures~\ref{fig:fsos}--\ref{fig:xc2sos}.
...
\end{example}
%
The next example illustrates a concerted transition involving a weak bond.
...
$$ 
We notice that, following the first transition, the $h[2]$ bond can be broken, and correspondingly the $c[1]$ bond can be broken after the second transition. Since creation and breaking of such bonds is almost instantaneous, we need a new concert rule that capture such reactions. Using \rulename{concert3} we derive the following:
$$
