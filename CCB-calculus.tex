...
{\sigma \wr \mu \xrightarrow{a[k]}_s \sigma' \wr \mu}
%
\\[25pt]
\rulename{act1}\ 
\Rule
...
{\sigma \wr \mu \xrightarrow{(b)[l]}_s \sigma' \wr \mu}
\\[20pt]
\rulename{aux1}\ 
...
We associate to $\Proc$ and $\mAK \cup \umAK \cup (\mAK \times \umAK) $
...
``shortcut'' SOS rules to capture such cascade of two concerted actions and a reversal of an unstable action, called \rulename{concert3}. They are not formally part of our operational semantics but could be used in some circumstances 
...
The current version of CCB is a minor extension of the original CCB from \cite{KU2017} by extending the 
prefix operator to use collections of sites. If collections have only one site, then both calculi have the same syntax. We are
using additionally shortcut rules for concerted transitions that involve three actions, namely \rulename{concert3}, \rulename{concert3 act}, \rulename{concert3 par}, \rulename{concert3 res}, but they are just a shorthand for sequences of two standard transitions.
%our standard rules. 

We have enlarged slightly the class of processes for which we can derive concerted actions and for that reason we have used 
...
All the properties that the original CCB possesses are also enjoyed by the current calculus.
...
%a DNA mismatch repair mechanism. 
